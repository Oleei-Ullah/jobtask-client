...
const Featured = () => {
    ...
    const { data: featuredProducts, isLoading, isError, refetch } = useQuery({
        queryKey: ['featuredProducts', sort],
        queryFn: () => fetch(`https://techshownew.vercel.app/api/product/?sort=${sort ? 'latest' : 'older'}`)
            .then(res => res.json())
    })
    ...
    if (featuredProducts.length) return (
        <div className="container mx-auto px-3 pt-20">
            ...
            <button onClick={() => setSort(!sort)} className="bg-gray-300 text-center mx-auto my-3 inline-block px-2 rounded-e-md min-w-[200px]">
                <h1 className="bg-gradient-to-r from-blue-600 via-green-500 to-indigo-400 text-transparent bg-clip-text font-bold inline-block text-2xl font-lora text-center">Sort By {sort ? "Oldest" : "Latest"}</h1>
            </button>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {
                    featuredProducts.map(({ img, _id, name, desc, timestamp, authorEmail, upvotes }) => {
